...
- [ ] Task 1: Project scaffold (AC: #1, #2, #3 — everything depends on this existing)
  - [ ] `npm init`; add dependencies pinned exactly per Stack table: `express@5.2.1`, `pg@8.22.0`, `node-pg-migrate@8.0.x` (verify exact patch on npmjs.com before pinning — sources disagreed on 8.0.3 vs 8.0.4, see Dev Notes)
  - [ ] Create the directory structure exactly as in ARCHITECTURE-SPINE.md's Structural Seed: `src/{db,routes,services,lib}`, `scripts/`, `migrations/`, `test/`
  - [ ] `package.json` scripts, fixed names (no others): `migrate` → `node-pg-migrate up`, `seed` → `node scripts/seed.js`, `start` → `node src/server.js`, `test` → `node --test`
  - [ ] `src/db/pool.js`: reads `DATABASE_URL` once, exports the shared `pg.Pool`. This is the only module that reads `DATABASE_URL` — nothing else calls `process.env` for DB config (AD-7)
- [ ] Task 2: Migration — create the `customers` table (AC: #1)
  - [ ] One `node-pg-migrate` migration file under `migrations/` creating `customers` with: `id` (serial PK), `name` (text, not null), `telepules` (text, not null), `lat` (double precision, nullable), `lon` (double precision, nullable), `budget` (numeric(10,2), nullable), `note` (text, nullable)
  - [ ] Add `UNIQUE (name, telepules)` constraint in the same migration — this is the sole idempotency mechanism (AD-4); do not add any application-level "does this row exist" check
- [ ] Task 3: `src/lib/normalizeCity.js` — pure normalization function (AC: #2)
  - [ ] Export a single function: lowercase, Unicode-normalize and strip diacritics (e.g. `"Kraków".normalize('NFD').replace(/[̀-ͯ]/g, '')` → `"krakow"`), trim whitespace
  - [ ] Special-case: any input containing "budapest" (post-normalization, e.g. to also catch a district name if one ever appears) resolves to matching `reference/city-coordinates.json`'s `"budapest"` entry
  - [ ] This is the ONLY place normalization logic lives — `src/services/seedService.js` calls it, nothing else re-implements it (AD-6)
  - [ ] **Path note:** lives under `src/lib/`, not a top-level `lib/` — matches ARCHITECTURE-SPINE.md's Structural Seed exactly
- [ ] Task 4: `src/services/seedService.js` + `scripts/seed.js` — the seed workflow (AC: #2, #3)
  - [ ] Load `seed-customers.json` and `reference/city-coordinates.json` (both already exist at repo root — do not regenerate or modify them)
  - [ ] For each seed customer: normalize `location.city` via `src/lib/normalizeCity.js` **for matching only** — the *normalized* value (e.g. `"krakow"`) is a lookup key, never what gets stored
  - [ ] **`telepules` must be stored as the original, unnormalized `location.city` string** (e.g. `"Kraków"`, not `"krakow"`) — Story 1.3's response and its name/telepules display depend on the human-readable form, not the lookup key
  - [ ] Look up the reference array by matching **both** `normalizedCity` AND `countryCode`. If `normalizedCity` matches but `countryCode` differs, treat as **no match** (`lat`/`lon = null`, log a warning) — this exact combination doesn't occur in the current 15-row dataset, but the matching logic must still enforce it since a future seed file could hit it
  - [ ] Match found → set `lat`/`lon` from the reference entry. No match → `lat`/`lon = null`, `console.warn` with the customer's name/city, continue to the next customer (never throw)
  - [ ] Insert via `INSERT ... ON CONFLICT (name, telepules) DO NOTHING` — do not pre-check existence in application code
  - [ ] `scripts/seed.js` is the standalone entrypoint (`npm run seed`) that calls `seedService`; it must NOT be invoked from `src/server.js` (AD-2 — server never seeds on boot)
- [ ] Task 5: Automated regression tests for AC #2 and #3 (uses `node:test`, per AD-8 — the runner is already established, don't defer these to manual-only checks)
  - [ ] Test: running the seed logic twice against the same database yields the same row count (AC #3)
  - [ ] Test: a customer whose city has no reference match ends up with `lat`/`lon = null` and does not throw (AC #2's miss path — not naturally exercised by the real 15-row dataset, see Dev Notes)
  - [ ] Test: `src/lib/normalizeCity.js` normalizes `"Kraków"` → `"krakow"` (accent-stripping sanity check)
